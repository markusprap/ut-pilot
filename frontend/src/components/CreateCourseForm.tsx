...
const CreateCourseForm: React.FC<CreateCourseFormProps> = ({ onSubmit, onCancel, isUploading }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg animate-in zoom-in duration-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all uppercase"
              disabled={isUploading}
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Upload Modul (PDF)</label>
            {!file ? (
              <div 
              ...
              >
                ...
                <p className="text-sm font-medium text-slate-700">Klik untuk upload PDF</p>
                <p className="text-xs text-slate-400 mt-1">Maksimal 50MB</p>
              </div>
            ) : (
              <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex items-center justify-between">
                <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500">{(file.size/1024/1024).toFixed(1)} MB • Siap upload</p>
                    </div>
                </div>
                <button 
                    type="button"
          ...
          <div className="pt-4 flex items-center justify-end gap-3">
            ...
            <button
              ...
              className="flex items-center px-6 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isUploading ? (
                <>
